...
# ---------------------------------------------------------------------------------------------------------------
# class Tmdb:
#   Scraper para streamondemand-PureITA basado en el Api de https://www.themoviedb.org/
#   version 1.4:
...
class Tmdb(object):
    ...
    def rellenar_dic_generos(cls, tipo='movie', idioma='it'):
        ...
        if tipo not in cls.dic_generos[idioma]:
            ...
            url = ('http://api.themoviedb.org/3/genre/%s/list?api_key=99ceb6cfe8e4ee644dc3b4d2ca2b2044&language=%s'
                   % (tipo, idioma))
            try:
    ...
    def __by_id(self, source='tmdb'):
        ...
        if self.busqueda_id:
            if source == "tmdb":
                ...
                #   &append_to_response=images,videos,external_ids,credits&include_image_language=es,null
                url = ('http://api.themoviedb.org/3/%s/%s?api_key=99ceb6cfe8e4ee644dc3b4d2ca2b2044&language=%s'
                ...
                url = ('http://api.themoviedb.org/3/find/%s?external_source=%s&api_key=99ceb6cfe8e4ee644dc3b4d2ca2b2044'
    ...
    def __search(self, index_results=0, page=1):
        ...
        if self.busqueda_texto:
            ...
            url = ('http://api.themoviedb.org/3/search/%s?api_key=99ceb6cfe8e4ee644dc3b4d2ca2b2044&query=%s&language=%s'
    ...
    def __discover(self, index_results=0):
        ...
        if type_search:
            ...
            url = ('http://api.themoviedb.org/3/%s?api_key=99ceb6cfe8e4ee644dc3b4d2ca2b2044&%s'
    ...
    def get_temporada(self, numtemporada=1):
        ...
        if not self.temporada.get(numtemporada,{}):
            ...
            url = "http://api.themoviedb.org/3/tv/%s/season/%s?api_key=99ceb6cfe8e4ee644dc3b4d2ca2b2044&language=%s" \
    ...
    def get_videos(self):
        ...
        if self.result['id']:
            if self.result['videos']:
                ...
                url = "http://api.themoviedb.org/3/%s/%s/videos?api_key=99ceb6cfe8e4ee644dc3b4d2ca2b2044&language=%s" \
